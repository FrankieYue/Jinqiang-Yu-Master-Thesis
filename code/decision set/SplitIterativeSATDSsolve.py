from util.SplitIterativeSATdssolver import SplitIterativeSATdsSolver
from util.SplitIterativeSATds import SplitIterativeSATds
from util.dataProcessing import data_processing
import statistics
import os

def SplitIterativeSATDSSolve(filename, no_ins, N):
    print("File: {0}\n".format(filename))
    ...
    for ins in range(no_ins):
        ...
        for class_ids in range(num_class):
            ...
            found = False
            while not found:
                DDS_solver = SplitIterativeSATdsSolver(K, N, data[class_ids])
                ...
                if solutions is not None:
                    ...
                    for sol in solutions:
                        ...
                        print("\nFor class {0}, the current number of nodes are: {1}".format(class_ids + 1,
                                                                                             N) + ". Solution found")
                        print("The number of literals: {0}, the number of rules: {1}".format(lit, N - lit))
                        ...
                        clauses.append(len(DDS_solver.cnf.clauses))
        ...
        ds = SplitIterativeSATds(K, sum(num_nodes))
        ds.com_draw(filename[:-4], sols, num_nodes)
        ds.com_validate(data_features, data_classes, num_nodes, num_class)
        ...
        for i in range(len(num_nodes)):
            print(
                "For class{0}, the number of nodes: {1}, the number of literals: {2}, the number of rules: {3}".format(
                    i + 1, num_nodes[i], num_literal[i], num_nodes[i] - num_literal[i]))

        print("The total number of nodes: {0}".format(sum(num_nodes)))
